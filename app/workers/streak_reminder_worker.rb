...
class StreakReminderWorker
  ...
  def perform
    start_time = Time.now
    puts "🔔 [Streak Reminder] Starting daily streak reminder job at #{start_time}"
    ...
    if users_to_remind.empty?
      puts "✅ [Streak Reminder] No users need reminders today"
      return
    end
    
    puts "📨 [Streak Reminder] Found #{users_to_remind.size} users to remind"
    ...
    users_to_remind.each do |user|
      begin
        ...
        puts "❌ [Streak Reminder] Error sending to user #{user['id']}: #{e.message}"
        error_count += 1
      end
    end
    
    duration = Time.now - start_time
    puts "✅ [Streak Reminder] Complete! Sent #{sent_count} reminders (#{error_count} errors) in #{duration.round(2)}s"
    ...
    puts "❌ [Streak Reminder] Job failed: #{e.message}"
