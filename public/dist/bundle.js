var N=(n,e)=>()=>(e||n((e={exports:{}}).exports,e),e.exports);var m=(n,e,t)=>new Promise((o,s)=>{var i=d=>{try{a(t.next(d))}catch(c){s(c)}},r=d=>{try{a(t.throw(d))}catch(c){s(c)}},a=d=>d.done?o(d.value):Promise.resolve(d.value).then(i,r);a((t=t.apply(n,e)).next())});var R=N((j,g)=>{window.handleMediaError=function(n){if(!n.dataset.errorHandled){n.dataset.errorHandled="true";const e=n.dataset.fallback;if(e&&n.src!==e){n.src=e;return}n.src="/images/meme-placeholder.svg",n.alt="Image failed to load",console.warn("Image failed to load:",n.dataset.originalSrc||n.src)}};class B{constructor(){this.currentIndex=0,this.images=[],this.init()}init(){console.log("[MemeDisplay] Initializing..."),this.bindCarouselControls(),this.setupImageErrorHandling()}bindCarouselControls(){const e=document.getElementById("carousel-prev"),t=document.getElementById("carousel-next");e&&e.addEventListener("click",()=>this.showPrevious()),t&&t.addEventListener("click",()=>this.showNext())}setupImageErrorHandling(){const e=document.getElementById("meme-image");e&&e.addEventListener("error",()=>this.handleImageError())}showPrevious(){this.currentIndex>0&&(this.currentIndex--,this.updateDisplay())}showNext(){this.currentIndex<this.images.length-1&&(this.currentIndex++,this.updateDisplay())}updateDisplay(){document.querySelectorAll(".gallery-slide").forEach((t,o)=>{t.classList.toggle("active",o===this.currentIndex)}),document.querySelectorAll(".gallery-dot").forEach((t,o)=>{t.classList.toggle("active",o===this.currentIndex)});const e=document.getElementById("carousel-counter")||document.querySelector(".gallery-counter");e&&this.images.length>1&&(e.textContent=`${this.currentIndex+1} / ${this.images.length}`,e.style.display="block"),console.log(`[MemeDisplay] Showing image ${this.currentIndex+1}/${this.images.length}`)}handleImageError(){console.warn("[MemeDisplay] Image failed to load"),typeof window.showPlaceholder=="function"&&window.showPlaceholder()}}class q{constructor(){this.loading=!1,this.prefetchedMeme=null,this.transitionDuration=300,this.init()}init(){console.log("[MemeNavigation] Initializing AJAX navigation..."),this.bindKeyboardShortcuts(),this.bindNavigationButtons(),this.setupPopStateHandler(),this.prefetchNext()}bindKeyboardShortcuts(){document.addEventListener("keydown",e=>this.handleKeyPress(e))}bindNavigationButtons(){document.querySelectorAll('[data-action="next-meme"], .next-button, #next-btn').forEach(o=>{o.addEventListener("click",s=>{s.preventDefault(),this.loadNextMeme()})}),document.querySelectorAll('[data-action="similar-meme"]').forEach(o=>{o.addEventListener("click",s=>{s.preventDefault();const i=o.dataset.subreddit;i&&this.loadSimilarMeme(i)})})}setupPopStateHandler(){window.addEventListener("popstate",e=>{e.state&&e.state.meme?this.renderMeme(e.state.meme,!1):window.location.reload()})}handleKeyPress(e){if(!this.isInputFocused())switch(e.code){case"Space":case"ArrowRight":e.preventDefault(),this.loadNextMeme();break;case"ArrowLeft":e.preventDefault(),window.history.back();break;case"KeyL":e.preventDefault(),this.triggerLike();break;case"KeyS":e.preventDefault(),this.triggerSave();break;case"KeyT":e.preventDefault(),this.toggleTitle();break}}isInputFocused(){const e=document.activeElement;return e&&(e.tagName==="INPUT"||e.tagName==="TEXTAREA"||e.isContentEditable)}loadNextMeme(){return m(this,null,function*(){if(this.loading){console.log("[MemeNavigation] Already loading, please wait...");return}console.log("[MemeNavigation] Loading next meme via AJAX..."),this.loading=!0;try{this.showLoadingState();let e;if(this.prefetchedMeme)console.log("[MemeNavigation] Using prefetched meme"),e=this.prefetchedMeme,this.prefetchedMeme=null;else{const t=yield fetch("/random.json");if(!t.ok)throw new Error(`HTTP ${t.status}: ${t.statusText}`);e=yield t.json()}yield this.renderMeme(e),this.updateURL(e),this.prefetchNext(),this.trackView(e),console.log("[MemeNavigation] ✅ Meme loaded successfully")}catch(e){console.error("[MemeNavigation] Failed to load meme:",e),this.showError("Failed to load meme. Please try again."),setTimeout(()=>{window.location.href="/random"},2e3)}finally{this.loading=!1,this.hideLoadingState()}})}loadSimilarMeme(e){return m(this,null,function*(){if(!this.loading){console.log(`[MemeNavigation] Loading similar meme from r/${e}...`),this.loading=!0;try{this.showLoadingState();const t=yield fetch(`/similar.json?subreddit=${encodeURIComponent(e)}`);if(!t.ok)throw new Error(`HTTP ${t.status}`);const o=yield t.json();yield this.renderMeme(o),this.updateURL(o),this.prefetchNext(),this.trackView(o)}catch(t){console.error("[MemeNavigation] Failed to load similar meme:",t),this.showError("No similar memes found. Showing random instead."),setTimeout(()=>this.loadNextMeme(),1e3)}finally{this.loading=!1,this.hideLoadingState()}}})}renderMeme(e,t=!0){return m(this,null,function*(){const o=document.querySelector("#meme-display"),s=document.querySelector("#meme-info");if(!o){console.error("[MemeNavigation] #meme-display not found");return}o.style.opacity="0",o.style.transition=`opacity ${this.transitionDuration}ms ease-out`,yield this.wait(this.transitionDuration),o.innerHTML=this.renderMemeHTML(e),s&&(s.innerHTML=this.renderInfoHTML(e)),this.updateControlsState(e),o.style.opacity="1",t&&this.updateURL(e),window.scrollTo({top:0,behavior:"smooth"})})}renderMemeHTML(e){var t,o;return e.media_type==="video"||(t=e.url)!=null&&t.includes("v.redd.it")?`
  ...
  `,n.appendChild(o)}function U(n,e){const t=`Check out this meme! ${n}`,o=`https://wa.me/?text=${encodeURIComponent(t+" "+e)}`;window.open(o,"_blank"),typeof trackEvent=="function"&&trackEvent("share","whatsapp",n)}function H(n,e){const t=`https://twitter.com/intent/tweet?text=${encodeURIComponent(n)}&url=${encodeURIComponent(e)}&hashtags=memes`;window.open(t,"_blank","width=550,height=420"),typeof trackEvent=="function"&&trackEvent("share","twitter",n)}function w(n){navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(n).then(()=>{L()}).catch(e=>{x(n)}):x(n),typeof trackEvent=="function"&&trackEvent("share","copy_link",n)}function x(n){const e=document.createElement("textarea");e.value=n,e.style.position="fixed",e.style.left="-999999px",e.style.top="-999999px",document.body.appendChild(e),e.focus(),e.select();try{document.execCommand("copy"),L()}catch(t){console.error("Failed to copy:",t),alert("Failed to copy link. Please copy manually: "+n)}document.body.removeChild(e)}function L(){const n=document.createElement("div");n.className="copy-toast",n.textContent="✓ Link copied to clipboard!",document.body.appendChild(n),setTimeout(()=>n.classList.add("show"),10),setTimeout(()=>{n.classList.remove("show"),setTimeout(()=>document.body.removeChild(n),300)},3e3)}function _(){document.querySelectorAll(".meme-actions").forEach(n=>{if(!n.querySelector(".share-btn")){const e=document.createElement("button");e.className="btn share-btn-primary",e.innerHTML="🔗 Share This Meme",e.onclick=function(){w(window.location.href)},n.appendChild(e)}})}function z(n,e,t){navigator.share?navigator.share({title:n,text:t||n,url:e}).then(()=>{typeof trackEvent=="function"&&trackEvent("share","native",n)}).catch(o=>console.log("Error sharing:",o)):w(e)}window.shareToWhatsApp=U;window.shareToTwitter=H;window.copyLink=w;window.nativeShare=z;(function(){const n="meme-explorer-theme";function e(){const r=localStorage.getItem(n);return r||(window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light")}function t(r){document.documentElement.setAttribute("data-theme",r),o(r)}function o(r){const a=document.getElementById("theme-toggle");a&&(a.textContent=r==="dark"?"☀️":"🌙",a.setAttribute("aria-label",`Switch to ${r==="dark"?"light":"dark"} mode`))}function s(){const a=(document.documentElement.getAttribute("data-theme")||e())==="dark"?"light":"dark";localStorage.setItem(n,a),t(a),typeof gtag!="undefined"&&gtag("event","theme_change",{theme:a})}function i(){const r=e();t(r),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(n)||t(a.matches?"dark":"light")})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",i):i(),window.toggleTheme=s})();class S{constructor(e){this.moduleName=e,this.errors=[]}wrap(e){return(...t)=>{try{return e(...t)}catch(o){return this.handleError(o),null}}}wrapAsync(e){return m(this,null,function*(){return(...t)=>m(this,null,function*(){try{return yield e(...t)}catch(o){return this.handleError(o),null}})})}handleError(e){console.error(`[${this.moduleName}] Error:`,e),window.AppLogger&&window.AppLogger.error({module:this.moduleName,error:e.message,stack:e.stack}),this.errors.push({timestamp:new Date,error:e.message,stack:e.stack}),this.showUserMessage()}showUserMessage(){if(sessionStorage.getItem(`error_shown_${this.moduleName}`))return;const e=`We encountered an issue with ${this.moduleName}. Please refresh the page.`;window.showToast?window.showToast(e,"error"):console.warn(e),sessionStorage.setItem(`error_shown_${this.moduleName}`,"true")}getErrors(){return this.errors}}typeof g!="undefined"&&g.exports?g.exports=S:window.ErrorBoundary=S;(function(){const n=typeof Sentry!="undefined";window.addEventListener("error",function(e){return console.error("[Global Error]",{message:e.message,filename:e.filename,lineno:e.lineno,colno:e.colno,error:e.error}),n&&Sentry.captureException(e.error||new Error(e.message)),!1}),window.addEventListener("unhandledrejection",function(e){console.error("[Unhandled Promise Rejection]",e.reason),n&&Sentry.captureException(e.reason)}),console.log("[Error Handler] Global error handler initialized")})();(function(){let n=null;const e="pwa-prompt-dismissed",t=3e4;function o(){const i=localStorage.getItem(e);if(!i)return!1;const r=new Date(i);return(Date.now()-r.getTime())/(1e3*60*60*24)<7}function s(){if(!n||o())return;const i=document.createElement("div");i.id="pwa-install-banner",i.className="pwa-install-banner",i.innerHTML=`
    ...
    `,document.body.appendChild(i),document.getElementById("pwa-install-btn").addEventListener("click",()=>m(this,null,function*(){i.remove(),n.prompt();const{outcome:r}=yield n.userChoice;typeof gtag!="undefined"&&gtag("event","pwa_install",{outcome:r}),n=null})),document.getElementById("pwa-dismiss-btn").addEventListener("click",()=>{i.remove(),localStorage.setItem(e,new Date().toISOString()),typeof gtag!="undefined"&&gtag("event","pwa_prompt_dismissed")})}window.addEventListener("beforeinstallprompt",i=>{i.preventDefault(),n=i,setTimeout(s,t)}),window.addEventListener("appinstalled",()=>{n=null,typeof gtag!="undefined"&&gtag("event","pwa_installed")})})();"serviceWorker"in navigator&&navigator.serviceWorker.getRegistrations().then(function(n){for(let e of n)e.unregister().then(function(t){console.log("[SW] Unregistered old service worker:",t)})}).then(function(){console.log("[SW] Reloading to register fresh service worker..."),window.location.reload(!0)});(function(){function n(){const e=document.getElementById("menuToggle"),t=document.getElementById("hamburgerMenu");!e||!t||(e.addEventListener("click",o=>{o.preventDefault(),o.stopPropagation(),t.classList.toggle("active")}),document.addEventListener("click",o=>{!t.contains(o.target)&&!e.contains(o.target)&&t.classList.remove("active")}),document.addEventListener("keydown",o=>{o.key==="Escape"&&t.classList.remove("active")}))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",n):n()})();(function(){const n={init(){this.setupListeners(),this.showHintsOnFirstVisit()},setupListeners(){document.addEventListener("keydown",t=>{if(!t.target.matches("input, textarea"))switch(t.key){case" ":t.preventDefault(),this.nextMeme();break;case"l":case"L":t.preventDefault(),this.likeMeme();break;case"s":case"S":t.preventDefault(),this.saveMeme();break;case"Escape":this.closeModals();break;case"ArrowLeft":this.previousMeme();break;case"ArrowRight":this.nextMeme();break;case"?":t.preventDefault(),this.showShortcutsHelp();break}})},nextMeme(){const t=document.querySelector('[data-action="next"], .next-meme-btn, #next-meme');t&&(t.click(),this.showFeedback("Next →"))},previousMeme(){const t=document.querySelector('[data-action="previous"], .prev-meme-btn');t&&(t.click(),this.showFeedback("← Previous"))},likeMeme(){const t=document.querySelector('[data-action="like"], .like-btn, #like-button');t&&(t.click(),this.showFeedback("❤️ Liked"))},saveMeme(){const t=document.querySelector('[data-action="save"], .save-btn, #save-button');t&&(t.click(),this.showFeedback("⭐ Saved"))},closeModals(){const t=document.querySelector('[data-dismiss="modal"], .close-modal');t&&t.click();const o=document.querySelector(".gamification-expanded");o&&(o.classList.remove("gamification-expanded"),o.classList.add("gamification-collapsed"))},showFeedback(t){const o=document.querySelector(".keyboard-feedback");o&&o.remove();const s=document.createElement("div");s.className="keyboard-feedback",s.textContent=t,s.style.cssText=`
        position: fixed;
        top: 20px;
        left: 50%;
        transform: translateX(-50%);
      ...
      `,document.body.appendChild(s),setTimeout(()=>s.remove(),1e3)},showHintsOnFirstVisit(){localStorage.getItem("keyboard-shortcuts-seen")||setTimeout(()=>{const t=document.createElement("div");t.className="keyboard-shortcuts-hint",t.innerHTML=`
          <div style="
            position: fixed;
            bottom: 80px;
            left: 50%;
        ...
        `,document.body.appendChild(t),setTimeout(()=>{t.remove(),localStorage.setItem("keyboard-shortcuts-seen","1")},8e3)},3e3)},showShortcutsHelp(){const t=document.createElement("div");t.className="shortcuts-help-modal",t.innerHTML=`
        <div style="
          position: fixed;
          top: 50%;
          left: 50%;
  ...
  `,document.head.appendChild(e),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>n.init()):n.init()})();function F(){const n=document.querySelector(".mobile-nav");n&&n.classList.toggle("open")}function M(n){document.documentElement.setAttribute("data-theme",n),localStorage.setItem("theme",n)}function v(){const n=localStorage.getItem("theme")||"light";M(n)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",v):v();typeof g!="undefined"&&g.exports&&(g.exports={toggleMobileNav:F,setTheme:M,initTheme:v});(function(){function n(){document.querySelectorAll(".meme-content-video").forEach(o=>{o.addEventListener("loadstart",function(){this.classList.add("loading")}),o.addEventListener("loadeddata",function(){this.classList.remove("loading")}),o.addEventListener("error",function(s){console.error("Video load error:",s),e(this)}),o.addEventListener("play",function(){typeof gtag!="undefined"&&gtag("event","video_play",{video_src:this.src})}),o.addEventListener("dblclick",function(){this.requestFullscreen?this.requestFullscreen():this.webkitRequestFullscreen&&this.webkitRequestFullscreen()})})}function e(t){var r;const o=t.parentElement,s=t.src,i=document.createElement("div");i.className="video-error",i.innerHTML=`
